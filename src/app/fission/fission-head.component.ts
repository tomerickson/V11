...
export class FissionHeadComponent implements OnInit {
  ...
  elements = signal<IElementDataModel[] | null>(
    this.asyncPipe.transform(this.store.select(globalFeature.selectElements))
  );
  sortFields = signal<ILookupDataModel[] | null>(
    this.asyncPipe.transform(
      this.store.select(globalFeature.selectReactionSortFields)
    )
  );
  ready = signal(false);
